...
const Orden = ({orden}) => {
    const {id,total,pedido,nombre} = orden
  ...
  return (
    <div className="border p-10 py-5">
        ...
        <div>
            {pedido.map(plato=>(
                <div key={plato.id} className="py-3 flex border-b last-of-type:border-0 items-center">
                    <div className="w-32">
                        <Image width={400} height={500} src={`/assets/img/${plato.imagen}.jpg`} alt={`imagen plato ${plato.nombre}`}/>
                    </div>
        ...
        <div className="md:flex md:items-center md:justify-between my-10">
            ...
            <button className="bg-indigo-600 hover:bg-indigo-800 text-white md:mt-0 py-3 px-10 uppercase font-bold rounded-lg" type="button" onClick={completarOrden}>Completar Orden</button>
        </div>
    </div>
  )
}
